...
// Кокосовая стружка
class CCocolate : public CCondimentDecorator
{
public:
	CCocolate(IBeveragePtr && beverage, unsigned count)
		: CCondimentDecorator(move(beverage))
		, m_count(count)
	{}
...
protected:
	...
	std::string GetCondimentDescription() const override
	{
		return "Cocolate  " + std::to_string(m_count) + "path ";
	}
private:
	unsigned m_count;
};
